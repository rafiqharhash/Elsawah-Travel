...
exports.exportTripBookings = exports.exportTripPDF = void 0;
...
// ─────────────────────────────────────────────────────────────────────────────
function hRule(doc, y, width, color = '#E2E8F0') {
...
const exportTripPDF = async (req, res, next) => {
    try {
        ...
        const trip = await Trip_1.Trip.findById(tripId).populate('vehicleIds', 'vehicleNumber driverName driverPhone capacity');
        if (!trip)
        ...
        for (const b of bookings) {
            ...
            if (!bookingsByVehicle.has(key))
                bookingsByVehicle.set(key, []);
        ...
        res.setHeader('Content-Disposition', `attachment; filename="trip_${tripId}_sheet.pdf"`);
        doc.pipe(res);
        ...
        const pageSources = vehicles.length > 0 ? [...vehicles] : [null];
        ...
        for (const vehicle of pageSources) {
            ...
            doc.font('Helvetica-Bold').fontSize(9).fillColor('#F1F5F9')
                .text(`Total Booked: ${booked}   ·   Available Seats: ${CAPACITY - booked}   ·   Capacity: ${CAPACITY}`, 55, y + 9);
            doc.font('Helvetica').fontSize(8).fillColor('#64748B')
                .text(`Generated on ${new Date().toLocaleString()}`, 55, y + 9, { align: 'right', width: PAGE_W - 15 });
            // ── Page number ──────────────────────────────────────────────────────
            doc.font('Helvetica').fontSize(8).fillColor('#94A3B8')
                .text(`Page ${pageIndex} of ${totalPages}`, 0, doc.page.height - 30, { align: 'center', width: doc.page.width });
        }
        doc.end();
    }
    catch (error) {
...
exports.exportTripPDF = exportTripPDF;
// ─────────────────────────────────────────────────────────────────────────────
// @desc  Export trip bookings as Excel or CSV
// @route GET /api/v1/export/trip/:tripId
// ─────────────────────────────────────────────────────────────────────────────
const exportTripBookings = async (req, res, next) => {
    const { tripId } = req.params;
    const { format } = req.query;
    const trip = await Trip_1.Trip.findById(tripId);
    if (!trip)
        return next(new errorHandler_1.AppError('Trip not found', 404));
    const bookings = await Booking_1.Booking.find({ tripId }).populate('vehicleId', 'vehicleNumber driverName');
    const data = bookings.map((b) => ({
        ...
        Vehicle: b.vehicleId?.vehicleNumber || 'N/A',
    ...
    if (format === 'excel') {
        ...
        const csv = data.length > 0 ? (0, json2csv_1.parse)(data) : 'No bookings found';
        res.header('Content-Type', 'text/csv');
        res.attachment(`trip_${tripId}_bookings.csv`);
        return res.send(csv);
    }
};
exports.exportTripBookings = exportTripBookings;
